...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet = {
      val isLeftEmpty = left.isInstanceOf[Empty]
      val isRightEmpty = right.isInstanceOf[Empty]
      ...
      if (!isLeftEmpty && !isRightEmpty) {
        ...
        else if (leftMost.retweets > rightMost.retweets) left.mostRetweeted
        else right.mostRetweeted
      } else elem
    }

    def descendingByRetweet: TweetList = {
      val retweet = mostRetweeted
      new Cons(retweet, remove(retweet).descendingByRetweet)
    }
