...
object MovieRecommendationsALS {
  ...
  def main(args: Array[String]) {
    ...
    val ratings = data
        .map(x => x.split('\t')).map( x => Rating(x(0).toInt, x(1).toInt, x(2).toDouble))
        .cache()
    ...
    for (rating <- myRatings) {
      println(nameDict.value(rating.product.toInt) + ": " + rating.rating.toString)
    }
    ...
    for (recommendation <- recommendations) {
      println( nameDict.value(recommendation.product.toInt) + " score " + recommendation.rating )
    }
